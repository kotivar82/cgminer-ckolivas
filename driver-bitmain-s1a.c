...
static void ants1a_detect(__maybe_unused bool hotplug)
{
}
			...
			if (tmp > 0)
				*chain_num = tmp;
				...
				quit(1, "Invalid bitmain-options for "
						"chain_num (%s) must be 1 ~ %d", colon,
						BITMAIN_DEFAULT_CHAIN_NUM);
			}
		}
			...
			if (colon3 && *colon3) {
				...
				if (tmp > 0 && tmp <= 0xff)
					*timeout = tmp;
					...
					quit(1, "Invalid bitmain-options for "
							"timeout (%s) must be 1 ~ %d", colon3, 0xff);
				}
				if (colon4 && *colon4) {
					...
					if (colon5 && *colon5) {
						if (strlen(colon5) > 8 || strlen(colon5) % 2 != 0
								|| strlen(colon5) / 2 == 0) {
							quit(1, "Invalid bitmain-options for"
									" reg data, must be hex now: %s", colon5);
						}
						memset(reg_data, 0, 4);
						if (!hex2bin(reg_data, colon5, strlen(colon5) / 2)) {
							quit(1, "Invalid bitmain-options for reg"
									" data, hex2bin error now: %s", colon5);
						}
					}
				}
			}
...
static int bitmain_set_txconfig(struct bitmain_txconfig_token *bm,
		...
		uint8_t frequency_eft, uint8_t voltage_eft,
		uint8_t chain_check_time_eft, uint8_t chip_config_eft,
		uint8_t hw_error_eft, uint8_t chain_num, uint8_t asic_num,
		uint8_t fan_pwm_data, uint8_t timeout_data, uint16_t frequency,
		uint8_t voltage, uint8_t chain_check_time, uint8_t chip_address,
		uint8_t reg_address, uint8_t * reg_data)
{
	...
	if (unlikely(timeout_data <= 0 || asic_num <= 0 || chain_num <= 0)) {
		applog(LOG_WARNING, "%s: %s() parameter invalid"
				" timeout_data(%d) asic_num(%d) chain_num(%d)", ANTDRV.dname,
				__func__, (int )timeout_data, (int )asic_num, (int )chain_num);
		return -1;
	}
	...
	applogsiz(LOG_DEBUG, 512, "%s: %s() reset(%d) faneft(%d) touteft(%d) freqeft(%d)"
			...
			(int)chip_config_eft, (int)hw_error_eft, (int)chain_num,
			(int)asic_num, (int)fan_pwm_data, (int)timeout_data,
			(int)frequency, (int)voltage, (int)chain_check_time,
			(int)reg_data[0], (int)reg_data[1], (int)reg_data[2],
			(int)reg_data[3], (int)chip_address, (int)reg_address,
			(int)crc);
...
static int bitmain_set_txtask(struct bitmain_info *info, uint8_t *sendbuf,
	...
	*sentcount = 0;
	...
	datalen = 10;
	applog(LOG_DEBUG, "%s: send work count %d", ANTDRV.dname,
			info->work_ready->count);
	while (info->work_ready->count) {
		witem = k_unlink_tail(info->work_ready); /* was head */
		if (DATAW(witem)->work->work_block > *last_work_block) {
			applog(LOG_ERR, "%s: send task new block %d old(%d)", ANTDRV.dname,
					DATAW(witem)->work->work_block, *last_work_block);
			new_block = 1;
			*last_work_block = DATAW(witem)->work->work_block;
		}
		work_id = DATAW(witem)->work->id; /* was changed from work->id to wid */
		bm->works[cursentcount].work_id = htole32(work_id);
		applog(LOG_DEBUG, "%s: send task work id:%"PRIu32" %"PRIu32,
				ANTDRV.dname, bm->works[cursentcount].work_id, work_id);
	...
	applog(LOG_DEBUG, "%s: TxTask Token: %d %d %02x%02x%02x%02x%02x%02x",
			ANTDRV.dname, datalen, bm->length, sendbuf[0], sendbuf[1],
...
static int bitmain_set_rxstatus(struct bitmain_rxstatus_token *bm,
		uint8_t chip_status_eft, uint8_t detect_get, uint8_t chip_address,
		uint8_t reg_address) {
	uint16_t crc = 0;
	int datalen = 0;
	uint8_t *sendbuf = (uint8_t *) bm;
...
static int bitmain_parse_rxstatus(const uint8_t * data, int datalen,
	...
	if (unlikely(!data || datalen <= 0)) {
		applog(LOG_ERR, "%s: %s() parameter invalid data is null"
				" or datalen(%d) error", ANTDRV.dname, __func__, datalen);
		return -1;
	}
	...
	for (i = 0; i < bm->temp_num; i++) {
		applog(LOG_DEBUG, "%s: RxStatus Data temp(%d) temp=%d", ANTDRV.dname,
				i + 1, bm->temp[i]);
	}
	applog(LOG_DEBUG, "%s: RxStatus Data fan info:", ANTDRV.dname);
	for (i = 0; i < bm->fan_num; i++) {
		applog(LOG_DEBUG, "%s: RxStatus Data fan(%d) fan=%d", ANTDRV.dname,
				i + 1, bm->fan[i]);
	}
	return 0;
}

static int bitmain_parse_rxnonce(const uint8_t * data, int datalen,
		struct bitmain_rxnonce_data *bm, int * nonce_num) {
	int i = 0;
	uint16_t crc = 0;
	int curnoncenum = 0;
	if (unlikely(!bm)) {
	...
	if (bm->data_type != BITMAIN_DATA_TYPE_RXNONCE) {
		applog(LOG_ERR, "%s: %s() datatype(%02x) error", ANTDRV.dname, __func__,
				bm->data_type);
		return -1;
	}
	if (bm->length+2 != datalen) {
...
static int bitmain_read(struct cgpu_info *bitmain, unsigned char *buf,
	...
	if (is_usb) {
		err = usb_read_once_timeout(bitmain, (char * )buf, bufsize, &readlen,
				timeout, ep);
		applog(LOG_DEBUG, "%s%i: Get %s() got readlen %d err %d",
				bitmain->drv->name, bitmain->device_id, __func__, readlen, err);
	}
	return readlen;
}
...
static int bitmain_write(struct cgpu_info *bitmain, char *buf, ssize_t len,
	...
	if (is_usb) {
		...
		applog(LOG_DEBUG, "%s%d: usb_write got err %d", bitmain->drv->name,
		...
		if (amount != len) {
			applog(LOG_ERR, "%s%d: usb_write length mismatch on %s() "
					"amount=%d len=%d", bitmain->drv->name, bitmain->device_id,
					__func__, amount, (int )len);
			return BTM_SEND_ERROR;
		}
	}
	return BTM_SEND_OK;
...
static int bitmain_send_data(const uint8_t *data, int datalen,
		__maybe_unused struct cgpu_info *bitmain) {
	...
	applog(LOG_DEBUG, "%s: %s() stop ret=%d datalen=%d", ANTDRV.dname, __func__,
			ret, datalen);
...
static void bitmain_inc_nvw(struct bitmain_info *info, struct thr_info *thr) {
	applog(LOG_INFO, "%s%d: No matching work - HW error", thr->cgpu->drv->name,
			thr->cgpu->device_id);
...
static inline void record_temp_fan(struct bitmain_info *info,
		struct bitmain_rxstatus_data *bm, double *temp_avg) {
	int i = 0;
...
static void bitmain_update_temps(struct cgpu_info *bitmain,
		struct bitmain_info *info, struct bitmain_rxstatus_data *bm) {
	char tmp[64] = { 0 };
	char msg[10240] = { 0 };
	int i = 0;
	...
	applog(LOG_DEBUG, "%s%d: temp_index: %d, temp_count: %d, temp_max: %d",
			bitmain->drv->name, bitmain->device_id, info->temp_history_index,
			info->temp_history_count, info->temp_max);
	...
	if (unlikely(info->temp_hi >= opt_bitmain_overheat)) {
		...
		applog(LOG_WARNING, "%s%d: cooled, restarting", bitmain->drv->name,
				bitmain->device_id);
		info->overheat = false;
...
static void bitmain_parse_results(struct cgpu_info *bitmain,
		struct bitmain_info *info, struct thr_info *thr, uint8_t *buf,
		int *offset) {
		...
		if (buf[i] == 0xa1) {
			...
			if (bitmain_parse_rxstatus(buf+i, buf[i+1]+2, &rxstatusdata) != 0) {
				...
				for (n = 0; n < rxstatusdata.chain_num; n++) {
					...
					applog(LOG_DEBUG, "%s%d: %s() RxStatus Data chain(%d)"
							...
							bitmain->drv->name, bitmain->device_id, __func__, n,
							info->chain_asic_num[n], info->chain_asic_status[n],
			...
			if (spare > *offset) {
				applog(LOG_ERR, "%s%d: %s() spare(%d) > offset(%d)",
						bitmain->drv->name, bitmain->device_id, __func__, spare,
						*offset);
				spare = *offset;
			}
			break;
			...
			int nonce_num = 0;
			applog(LOG_DEBUG, "%s%d: %s() RxNonce Data", bitmain->drv->name,
					bitmain->device_id, __func__);
			if (*offset < 2) {
				return;
			}
			if (buf[i+1] > 70) {
			...
			if (bitmain_parse_rxnonce(buf+i, buf[i+1]+2, &rxnoncedata, &nonce_num) != 0) {
				...
				for (j = 0; j < nonce_num; j++) {
					...
					if (witem) {
						...
						/* how often is a duplicate nonce encountered? */
						if (isdupnonce(bitmain, work, rxnoncedata.nonces[j].nonce)) {
							// ignore it
							applog(LOG_ERR, "%s%d: duplicate nonce=%d",
								bitmain->drv->name, bitmain->device_id, rxnoncedata.nonces[j].nonce);
						} else {
							if (submit_nonce(thr, work, rxnoncedata.nonces[j].nonce)) {
								...
								applog(LOG_ERR, "%s%d: %s() RxNonce Data error work(%"PRIu32")",
										bitmain->drv->name, bitmain->device_id,
						...
						if (info->work_list->tail) {
							min = DATAW(info->work_list->tail)->work->id; /* wid? */
							max = DATAW(info->work_list->head)->work->id; /* wid? */
...
/*
						applog(LOG_ERR,
								"%s%d: %s() Work not found for id (%"PRIu32")",
								bitmain->drv->name, bitmain->device_id,
								__func__, rxnoncedata.nonces[j].work_id);
*/
					}
			...
			applog(LOG_ERR, "%s%d: %s() data type error=%02x",
					bitmain->drv->name, bitmain->device_id, __func__, buf[i]);
		}
	}
	if (!found) {
...
static void *bitmain_get_results(void *userdata) {
	...
	while (likely(!bitmain->shutdown)) {
		...
		applog(LOG_DEBUG, "%s%d: %s() offset=%d", bitmain->drv->name,
				bitmain->device_id, __func__, offset);

		if (offset >= (int) BITMAIN_READ_SIZE) {
			applog(LOG_DEBUG, "%s%d: %s() start", bitmain->drv->name,
					bitmain->device_id, __func__);
			bitmain_parse_results(bitmain, info, thr, (uint8_t *) readbuf,
					&offset);
			applog(LOG_DEBUG, "%s%d: %s() stop", bitmain->drv->name,
					bitmain->device_id, __func__);
		}
		...
		applog(LOG_DEBUG, "%s%d: %s() read", bitmain->drv->name,
				bitmain->device_id, __func__);
		ret = bitmain_read(bitmain, buf, rsize, BITMAIN_READ_TIMEOUT,
				C_BITMAIN_READ);
		applog(LOG_DEBUG, "%s%d: %s() read=%d", bitmain->drv->name,
				bitmain->device_id, __func__, ret);
...
//						bitmain->drv->name, bitmain->device_id, ret);
		...
		if (opt_debug) {
			applog(LOG_DEBUG, "%s%d: get:", bitmain->drv->name,
					bitmain->device_id);
...
static void bitmain_init(struct cgpu_info *bitmain) {
	applog(LOG_INFO, "%s%d: opened on %s", bitmain->drv->name,
			bitmain->device_id, bitmain->device_path);
}
...
static bool bitmain_prepare(struct thr_info *thr) {
	...
	if (pthread_create(&info->read_thr, NULL, bitmain_get_results,
			(void *) bitmain))
...
static int bitmain_initialize(struct cgpu_info *bitmain) {
	...
	if (!bitmain) {
		applog(LOG_WARNING, "%s%d: %s() cgpu_info is null", bitmain->drv->name,
				bitmain->device_id, __func__);
		return -1;
	}
	info = bitmain->device_data;
	...
	if (ret > 0) {
		if (opt_debug) {
			applog(LOG_DEBUG, "%s%d: clear read(%d):", bitmain->drv->name,
					bitmain->device_id, ret);
			hexdump(data, ret);
		}
	}

	sendlen = bitmain_set_rxstatus((struct bitmain_rxstatus_token *) sendbuf, 0,
			1, 0, 0);
	if (sendlen <= 0) {
		applog(LOG_ERR, "%s%d: %s() set_rx error(%d)", bitmain->drv->name,
				bitmain->device_id, __func__, sendlen);
		return -1;
	}

	ret = bitmain_send_data(sendbuf, sendlen, bitmain);
	if (unlikely(ret == BTM_SEND_ERROR)) {
		applog(LOG_ERR, "%s%d: %s() send_data error", bitmain->drv->name,
				bitmain->device_id, __func__);
		return -1;
	}
	while (trycount >= 0) {
	...
	if (statusok) {
		applog(LOG_ERR, "%s%d: %s() set_txconfig", bitmain->drv->name,
				bitmain->device_id, __func__);
		...
		sendlen = bitmain_set_txconfig(
				(struct bitmain_txconfig_token *)sendbuf, 1, 1, 1, 1, 1, 0, 1,
				eft, info->chain_num, info->asic_num,
		...
		applog(LOG_WARNING, "%s%d: %s() succeeded", bitmain->drv->name,
				bitmain->device_id, __func__);
	} else {
		applog(LOG_WARNING, "%s%d: %s() failed", bitmain->drv->name,
				bitmain->device_id, __func__);
		return -1;
	}
	return 0;
...
/* TODO: Begin ++ */
static struct cgpu_info *bitmain_detect_one(libusb_device *dev,
	...
	configured = get_options(this_option_offset, &baud, &chain_num, &asic_num,
			&timeout, &frequency, reg_data);
	...
	info->temp_history_count = (4
			/ (float) ((float) info->timeout * ((float) 1.67 / 0x32))) + 1;
	...
	applog(LOG_ERR, "%s: detected %s%d", ANTDRV.dname, bitmain->drv->name,
			bitmain->device_id);
	...
	info->work_list = k_new_list("Work", sizeof(WITEM), ALLOC_WITEMS,
			LIMIT_WITEMS, true);
	...
	applog(LOG_DEBUG, "%s%d: detected %s "
			...
			info->chain_num, info->asic_num, info->timeout, info->frequency);
...
static void ants1a_detect(bool __maybe_unused hotplug) {
	is_usb = true;
...
static void get_bitmain_statline_before(char *buf, size_t bufsiz,
		struct cgpu_info *bitmain) {
	struct bitmain_info *info = bitmain->device_data;
	int lowfan = 10000;
	int i = 0;
	...
	tailsprintf(buf, bufsiz, "%2d/%3dC %04dR", info->temp_avg, info->temp_max,
			lowfan);
}
...
static bool bitmain_fill(struct cgpu_info *bitmain) {
	...
	bool ret = true;
	int sendret = 0, sentcount = 0, neednum = 0, queuednum = 0, sendnum = 0,
			sendlen = 0;
	int roll, roll_limit;
	uint8_t sendbuf[BITMAIN_SENDBUF_SIZE];
	cgtimer_t ts_start;
	...
	if (info->fifo_space <= 0) {
		applog(LOG_DEBUG, "%s%d: %s() fifo space empty", bitmain->drv->name,
				bitmain->device_id, __func__);
		ret = true;
		goto out_unlock;
	}
	...
	while (info->fifo_space > 0) {
		...
		applog(LOG_DEBUG,
				"%s%d: Work task queued(%d) fifo space(%d) needsend(%d)",
				bitmain->drv->name, bitmain->device_id, queuednum,
				info->fifo_space, neednum);
			...
			if (!work)
				...
				while (queuednum < neednum && roll <= roll_limit) {
					applog(LOG_DEBUG, "%s%d: get work queued number:%d"
							" neednum:%d", bitmain->drv->name,
							bitmain->device_id, queuednum, neednum);
					...
					//DATAW(witem)->work->id = ++info->last_wid; /* was not here */
					//info->queued++; /* was not here */
					k_add_head(info->work_ready, witem); /* was tail */
					queuednum++;
					roll++;
				}
			}
		...
		applog(LOG_DEBUG, "%s%d: Send work %d", bitmain->drv->name,
				bitmain->device_id, sentcount);
		if (sendlen > 0) {
			...
			if (unlikely(sendret == BTM_SEND_ERROR)) {
				...
				if (info->errorcount > 1000) {
					...
					applog(LOG_ERR, "%s%d: Device disappeared,"
							" shutting down thread", bitmain->drv->name,
							bitmain->device_id);
					bitmain->shutdown = true;
				}
				break;
			} else {
				applog(LOG_DEBUG, "%s%d: send_data ret=%d", bitmain->drv->name,
						bitmain->device_id, sendret);
				info->errorcount = 0;
			}
		} else {
			applog(LOG_DEBUG, "%s%d: Send work set_txtask error: %d",
					bitmain->drv->name, bitmain->device_id, sendlen);
			break;
		}
	}

	out_unlock: cgtime(&now);
	...
	if (now.tv_sec - info->last_status_time.tv_sec > BITMAIN_SEND_STATUS_TIME) {
		applog(LOG_DEBUG,
				"%s%d: Send RX Status Token fifo_space(%d) timediff(%d)",
				bitmain->drv->name, bitmain->device_id, info->fifo_space,
				timediff);
		copy_time(&(info->last_status_time), &now);

		sendlen = bitmain_set_rxstatus(
				(struct bitmain_rxstatus_token *) sendbuf, 0, 0, 0, 0);
		if (sendlen > 0) {
			sendret = bitmain_send_data(sendbuf, sendlen, bitmain);
			if (unlikely(sendret == BTM_SEND_ERROR)) {
...
static void bitmain_flush_work(struct cgpu_info *bitmain) {
	...
	applog(LOG_ERR, "%s%d: %s() queued=%d", bitmain->drv->name,
			bitmain->device_id, __func__, info->queued);
	mutex_lock(&info->qlock);
	/* Will overwrite any work queued */
	info->queued = 0;
	k_list_transfer_to_tail(info->work_ready, info->work_list); /* was added */
...
static struct api_data *bitmain_api_stats(struct cgpu_info *cgpu) {
	...
	//int i = 0;
	double hwp =
			(cgpu->hw_errors + cgpu->diff1) ?
					(double) (cgpu->hw_errors)
							/ (double) (cgpu->hw_errors + cgpu->diff1) :
					0;
	...
	float avg =
			info->work_search ?
					(float) (info->tot_search) / (float) (info->work_search) :
					0;
	...
	avg = info->failed_search ?
			(float) (info->tot_failed) / (float) (info->failed_search) : 0;
...
struct device_drv ANTDRV = {
	.drv_id = DRIVER_ants1a,
	.dname = "BitmainAntS1a",
	.name = "S1A",
	.drv_detect = ants1a_detect,
#ifdef LINUX
